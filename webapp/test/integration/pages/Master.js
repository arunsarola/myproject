			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Peopleid") === oObject.Peopleid;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Peopleid;
									} else {
										sCurrentId = aItemsNotInTheList[0].Peopleid;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("nesspeopleSet", {
										Peopleid : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Peopleid"),
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<nesspeopleSet> (0)"}),
						success : function () {
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all nesspeopleSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<nesspeopleSet> (" + iExpectedLength + ")"}),
								success : function () {
